...
void update_next_pawn_position(Game *game, int *current_overflow_index, int i, int dx, int dy)
{
    ...
    if (game->current_turn == game->playerA)
        other_player = game->playerB;
    else
        other_player = game->playerA;
...
bool intersect(Point a, Point b, Point c, Point d)
{
    ...
    { // ab vertical et cd horizontal
        if (((a.x + b.x) / 2 == (c.x + d.x) / 2) && ((a.y + b.y) / 2 == (c.y + d.y) / 2))
            return true;
    }
    ...
    else if (a.y == b.y && b.y == c.y && c.y == d.y)
    { // all horizontal
        // need to check if they have a common point
        if (a.x > b.x)
            swap_points(&a, &b);
...
void draw_players(App *app)
{
    ...
    Player *a, *b;
    if (app->game->dragged_player == app->game->playerA)
    {
        a = app->game->playerB;
        b = app->game->playerA;
    }
    ...
    {
        player_pos_to_pixel_pos(a, &p);
    }
    if (a == app->game->playerA)
        SDL_SetRenderDrawColor(app->renderer, PLAYERA_COLOR);
    else
        SDL_SetRenderDrawColor(app->renderer, PLAYERB_COLOR);
    SDL_RenderFillCircle(app->renderer, p.x, p.y, PLAYER_RADIUS);
    ...
    {
        player_pos_to_pixel_pos(b, &p);
    }
    if (b == app->game->playerA) SDL_SetRenderDrawColor(app->renderer, PLAYERA_COLOR);
